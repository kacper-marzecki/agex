...
object MapTyper {
    ...
    for {
      tmp <- ZIO.foldRight(emap.kvs)(
        ...
        for {
          ...
          mapping =
            if (shouldHaveOptionalMapping(kTyped._type, gamma)) Optional.apply _
            else Required.apply _
      ...
      (kvs, mappings, ctx) = tmp
    } yield (TEMap(kvs, Type.TMap(mappings)), ctx)
  }
  ...
  def shouldHaveOptionalMapping(_type: Type, context: Context): Boolean = {
    ...
    _type match {
      ...
      case Type.TMap(kvs) =>
        containsOptionalType(kvs.flatMap(it => List(it.k, it.v)))

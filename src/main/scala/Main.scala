...
// Figure 11. Algorithmic typing
  def synthesizesTo(
    ...
    type Result = Eff[(TypedExpression, Context)]
    expr match {
      //1I=>
      ...
      case EVariable(name) => {
        context
          .getAnnotation(name)
          .fold[Result](fail(AnnotationNotFound(context, name))) {
      ...
      case EList(values) => {
        for {
          ...
          case x :: xs =>
            (
              TEList(synthedValues.typed, TList(commonSupertype(x, xs))),
              context
            )
      ...
      case emap: EMap => MapTyper.mapSynthesizesTo(context, emap)
      ...
      case ECase(expr, matches) => {
        for {
          ...
          patternsAndBranches <- foreach(matches) { case (m, e) =>
            for {
              (typedPattern, bindings, t) <- getMatch(
                context,
                exprTyped._type,
                m
              )
    ...
    (pattern, exprType) match {
      ...
      case (PMap(kvsp), TMap(kvs)) =>
        for {
          ...
          patternsAndBindings <- foreach(kvsp) { case (kPattern, vPattern) =>
            for {
